...
int count_live_cell(int *a, size) {
    int count = 0;
...
void calculate_next_generation(int *a, int *b) {
    //init_canvas(b);
...
void test_count_live_cell(int *a, int expected_output) {
        int final_output = count_live_cell(a);
...
{   
    ...
    if(rank==0){
        printf("Processor %d has finished. This took %f seconds\n", rank, end-start);
    }
